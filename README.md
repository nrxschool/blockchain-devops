# Blockchain DevOps

## Módulo 1: Fundamentos do Blockchain e Hyperledger Besu

**Aula 1.1: Introdução ao Blockchain**

- Tópicos: Evolução das blockchain, diferenças entre arquiteturas de Bitcoin, Ethereum, Polygon, Solana, Polkadot, Near, Hyperledger, R3Corda.
- Ferramentas: Slides de apresentação, vídeos ilustrativos.
- Projeto: Discussão e análise comparativa das arquiteturas.

**Aula 1.2: Instalação e Configuração Inicial do Besu**

- Tópicos: Passo a passo para instalação local do Hyperledger Besu, exploração da interface de linha de comando.
- Ferramentas: Hyperledger Besu, Besu CLI
- Projeto: Instalação e configuração inicial de um node Besu no ambiente local do aluno.

## Módulo 2: Desenvolvimento e Implantação de Smart Contracts

**Aula 2.1: Smart Contracts no Ethereum**

- Tópicos: Ciclo de vida de desenvolvimento, build, teste e deploy de um smart contract ERC20.
- Ferramentas: Solidity, foundry, Metamask.
- Projeto: Criação, teste e deploy de um smart contract ERC20 em uma rede de teste Ethereum.

**Aula 2.2: JSON-RPC**

- Tópicos: Interagir com o Besu via JSON-RPC, métodos básicos, interação com smart contracts.
- Ferramentas: Postman, cURL, scripts personalizados.
- Projeto: Configuração e execução de chamadas RPC para interação com o node Besu.

**Aula 2.3: Integração e Automação de CI/CD**

- Tópicos: Introdução a CI/CD, automação de testes e deployment de smart contracts.
-Ferramentas: GitHub Actions.
- Projeto: Configuração de um pipeline de CI/CD para automação do processo de testes e deployment de smart contracts.

## Módulo 3: Arquitetura e Operações de Node Blockchain

**Aula 3.1: Configurando Docker e Dockerfiles para Besu**

- Tópicos: Preparação do Besu para execução em contêineres, criação de Dockerfiles.
- Ferramentas: Docker.
- Projeto: Desenvolvimento de um Dockerfile personalizado e deploy de um node Besu em contêiner.

**Aula 3.2: Kubernetes e Orquestração de Containers**

- Tópicos: Gerenciamento de clusters de nodes Besu com Kubernetes, uso de Helm para gerenciamento de pacotes.
- Ferramentas: Kubernetes, Helm.
- Projeto: Configuração de um cluster Kubernetes e deploy de nodes Besu usando Helm.

**Aula 3.3: Cloud**

- Tópicos: Visão geral dos serviços de cloud, comparação entre AWS, Azure, GCP, deploy do Besu no cloud.
- Ferramentas: AWS EC2, Azure VM, Google Compute Engine.
- Projeto: Criação de uma conta e deploy de um node Besu em um provedor de cloud escolhido pelo aluno.

**Aula 3.4: Deploy com CI/CD**

- Tópicos: Integração do CI/CD com serviços de cloud para deploy de smart contracts.
- Ferramentas: Github Actions.
- Projeto: Configuração de um pipeline CI/CD para deploy de smart contracts em um ambiente de cloud.

## Módulo 4: Monitoramento, Observabilidade e Segurança

**Aula 4.1: Coleta de Métricas**

- Tópicos: Importância das métricas, implementação da coleta de métricas com Prometheus.
- Ferramentas: Prometheus.
- Projeto: Configuração de Prometheus para monitorar um node Besu.

**Aula 4.2: Visualização das Métricas**

- Tópicos: Visualização das métricas coletadas utilizando Grafana.
- Ferramentas: Grafana
- Projeto: Criação de dashboards no Grafana para visualizar métricas de um node Besu.

**Aula 4.3: Visualização e Indexação de Dados**

- Tópicos: Uso de exploradores e indexadores de blockchain para visualização de dados.
- Ferramentas: Blockscout, The Graph.
- Projeto: Configuração de um explorador de blockchain e criação de subgrafos usando The Graph.

## Módulo 5: Escalabilidade e Segurança

**Aula 5.1: Técnicas de Escalabilidade para Blockchain**

- Tópicos: Conceitos de escalabilidade horizontal versus vertical, aplicados a nodes de blockchain em contêineres, Load Balancing, High Availability.
- Ferramentas: Documentação técnica, estudos de caso.
- Projeto: Levantar uma rede Besu em ambiente Kubernetes com suporte a HA

**Aula 5.2: Aplicando Técnicas de Escalabilidade e Testes de Carga no Kubernetes**

- Tópicos: Introdução ao teste de carga e stress, Alocação de recursos (CPU, memória), Monitoramento de carga
- Ferramentas: Kubernetes, K6, Caliper, Prometheus.
- Projeto: Realização de testes de carga e aplicação de ajustes de performance baseados nos resultados obtidos.

**Aula 5.3: Segurança de Contêineres**

- Tópicos: Melhores práticas de segurança para contêineres, exploração de vulnerabilidades comuns em contêineres, ferramentas de segurança específicas para Docker e Kubernetes.
- Ferramentas: Docker Bench for Security, Kubernetes Security Posture Management (KSPM) tools.
- Projeto: Auditoria de segurança em um contêiner Docker rodando um node Besu, implementação de correções baseadas nas recomendações de segurança.

**Aula 5.4: Segurança em Smart Contracts**

- Tópicos: Vulnerabilidades de smart contracts, estratégias de segurança em CI/CD e ambientes blockchain.
- Ferramentas: Slither, MythX.
- Projeto: Análise de segurança de smart contracts e implementação de medidas de segurança no pipeline de CI/CD.
